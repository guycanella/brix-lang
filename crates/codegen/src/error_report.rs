...
pub fn report_codegen_error(filename: &str, source: &str, error: &CodegenError) {
    match error {
        CodegenError::LLVMError {
            ...
            let report = Report::build(
                ReportKind::Error,
                filename,
                span.as_ref().map(|s| s.start).unwrap_or(0),
            )
            .with_code("E101")
...
pub fn report_codegen_errors(filename: &str, source: &str, errors: &[CodegenError]) {
    for error in errors {
