...
impl<'a, 'ctx> StringFunctions<'ctx> for Compiler<'a, 'ctx> {
    ...
    fn get_str_trim(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_trim") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let fn_type = ptr_type.fn_type(&[ptr_type.into()], false);
        self.module
            .add_function("brix_str_trim", fn_type, Some(Linkage::External))
    }

    fn get_str_ltrim(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_ltrim") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let fn_type = ptr_type.fn_type(&[ptr_type.into()], false);
        self.module
            .add_function("brix_str_ltrim", fn_type, Some(Linkage::External))
    }

    fn get_str_rtrim(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_rtrim") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let fn_type = ptr_type.fn_type(&[ptr_type.into()], false);
        self.module
            .add_function("brix_str_rtrim", fn_type, Some(Linkage::External))
    }

    fn get_str_starts_with(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_starts_with") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let i64_type = self.context.i64_type();
        ...
        self.module
            .add_function("brix_str_starts_with", fn_type, Some(Linkage::External))
    }

    fn get_str_ends_with(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_ends_with") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let i64_type = self.context.i64_type();
        let fn_type = i64_type.fn_type(&[ptr_type.into(), ptr_type.into()], false);
        self.module
            .add_function("brix_str_ends_with", fn_type, Some(Linkage::External))
    }

    fn get_str_contains(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_contains") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let i64_type = self.context.i64_type();
        let fn_type = i64_type.fn_type(&[ptr_type.into(), ptr_type.into()], false);
        self.module
            .add_function("brix_str_contains", fn_type, Some(Linkage::External))
    }

    fn get_str_substring(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_substring") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let i64_type = self.context.i64_type();
        ...
        self.module
            .add_function("brix_str_substring", fn_type, Some(Linkage::External))
    }

    fn get_str_reverse(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_reverse") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let fn_type = ptr_type.fn_type(&[ptr_type.into()], false);
        self.module
            .add_function("brix_str_reverse", fn_type, Some(Linkage::External))
    }

    fn get_str_repeat(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_repeat") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let i64_type = self.context.i64_type();
        ...
        self.module
            .add_function("brix_str_repeat", fn_type, Some(Linkage::External))
    }

    fn get_str_index_of(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_index_of") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let i64_type = self.context.i64_type();
        let fn_type = i64_type.fn_type(&[ptr_type.into(), ptr_type.into()], false);
        self.module
            .add_function("brix_str_index_of", fn_type, Some(Linkage::External))
    }

    fn get_str_char_at(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_char_at") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        let i64_type = self.context.i64_type();
        ...
        let fn_type = ptr_type.fn_type(&[ptr_type.into(), i64_type.into()], false);
        self.module
            .add_function("brix_str_char_at", fn_type, Some(Linkage::External))
    }

    // ===== v1.7 StringMatrix (Group A) =====

    fn get_str_split(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_split") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        ...
        self.module
            .add_function("brix_str_split", fn_type, Some(Linkage::External))
    }

    fn get_str_join(&self) -> inkwell::values::FunctionValue<'ctx> {
        if let Some(func) = self.module.get_function("brix_str_join") {
            return func;
        }
        let ptr_type = self.context.ptr_type(AddressSpace::default());
        ...
        let fn_type = ptr_type.fn_type(&[ptr_type.into(), ptr_type.into()], false);
        self.module
            .add_function("brix_str_join", fn_type, Some(Linkage::External))
    }
}
