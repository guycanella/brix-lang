...
├── runtime.c                # C runtime (~4,170 lines) — must be in project root
├── crates/
...
│   └── codegen/src/
│       ├── lib.rs           # Main compiler (~15,942 lines — post-refactor 11,014, grew with v1.8 + rustfmt expansion)
│       ├── stmt.rs          # Statement compilation (~1,344 lines)
...
**Current test baseline (v1.8 in progress — Grupos A/B/C/D COMPLETE, Grupos E/F not started):** 1,310 unit (317 lexer + 205 parser + 788 codegen) + 214 integration + 482 Test Library (28 `.test.bx` files). All green.
...
**Grupos E/F — NOT STARTED:** `MinHeap`/`MaxHeap`, `HashMap<K,V>` (both depend on `Vector<T>`, which is now complete).
