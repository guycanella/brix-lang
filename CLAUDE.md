...
├── runtime.c                # C runtime (~4,297 lines) — must be in project root
├── crates/
...
│   └── codegen/src/
│       ├── lib.rs           # Main compiler (~16,544 lines — post-refactor 11,014, grew with v1.8 + rustfmt expansion)
│       ├── stmt.rs          # Statement compilation (~1,349 lines)
...
**Current test baseline (v1.8 in progress — Grupos A/B/C/D/E COMPLETE, Grupo F not started):** 1,316 unit (317 lexer + 205 parser + 794 codegen) + 221 integration + 488 Test Library (28 `.test.bx` files). All green.
...
**Grupo F — NOT STARTED:** `HashMap<K,V>` (depends on `Vector<T>`, which is now complete — planned to use one internally for bucket storage).
